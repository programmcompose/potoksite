document$.subscribe(function() {
    ...
    const NODE_CONFIG = {
        CIRCLE_RADIUS_MIN: 5,
        CIRCLE_RADIUS_MAX: 28,
        LABEL_X_OFFSET: 10,
        LABEL_DY_OFFSET: ".35em",
        CURRENT_NODE_MULTIPLIER: 1.4,
    }
    ...
    function getNodeSection(node) {
        const id = (node.id || node.path || "").replace(/\\/g, "/");
        const parts = id.split("/");
        const firstPart = parts[0] || "";
        if (firstPart) return firstPart;
        // Handle root-level files
        const filename = (node.id || node.path || "").replace(/\\/g, "/").split("/").pop();
        if (filename === "faq.md") return "faq";
        if (filename === "glossary.md") return "glossary";
        return "";
    }

    function getNodeColor(node) {
        const section = getNodeSection(node);
        return SECTION_COLORS[section] || SECTION_COLORS[""];
    }

    function getNodeRadius(node, maxRadius, minRadius, maxDegree, minDegree, isCurrent) {
        const degree = node._degree || 0;
        const range = maxDegree - minDegree || 1;
        const radiusRange = maxRadius - minRadius;
        let radius = minRadius + ((degree - minDegree) / range) * radiusRange;
        if (isCurrent) radius *= NODE_CONFIG.CURRENT_NODE_MULTIPLIER;
        return Math.max(3, Math.min(radius, 32));
    }
       ...
       if (isModal) {
        ...
        if (isModal) {
 		...
 		const g = svg.append("g");
            ...
            Object.entries(SECTION_COLORS).forEach(([section, color]) => {
                defs.append("marker")
                    .attr("id", `arrowhead-${section}`)
                    .attr("class", "arrowhead")
                    .attr("viewBox", ARROWHEAD.VIEW_BOX)
                    .attr("refX", ARROWHEAD.REFX)
                    .attr("refY", ARROWHEAD.REFY)
            ...
            function getSectionMarkerId(section) {
                return `arrowhead-${section}`;
            }
        ...
        const draw = function(graph) {
           ...
           for (const link of linksData) {
            ...
            const processedLinks = Array.from(processedLinksMap.values());
             ...
             nodesData.forEach(n => { n._degree = degreeMap[n.id] || 0; });

             const degrees = nodesData.map(n => n._degree);
             ...
             const nodeRadius = function(d) {
                 return getNodeRadius(d, NODE_CONFIG.CIRCLE_RADIUS_MAX, NODE_CONFIG.CIRCLE_RADIUS_MIN, maxDegree, minDegree, d.id === startNode?.id);
             };
            ...
            if (isTwoNodes) {
              ...
              const link = g.append("g")
                  ...
                  .style("stroke-width", d => {
                      const sDeg = degreeMap[d.source] || 1;
                      const tDeg = degreeMap[d.target] || 1;
                      const sourceId = typeof d.source === 'object' ? d.source.id : d.source;
                      const targetId = typeof d.target === 'object' ? d.target.id : d.target;
                      const sourceNode = nodesData.find(n => n.id === sourceId);
                      const targetNode = nodesData.find(n => n.id === targetId);
                      const sameSection = sourceNode && targetNode && getNodeSection(sourceNode) === getNodeSection(targetNode);
                      if (sameSection) {
                          return Math.max(0.8, Math.min(sDeg, tDeg) * 0.2);
                      }
                      return Math.max(1.5, Math.min(sDeg, tDeg) * 0.4);
                  })
                  .style("stroke", d => {
                      const sourceId = typeof d.source === 'object' ? d.source.id : d.source;
                      const targetId = typeof d.target === 'object' ? d.target.id : d.target;
                  ...
                  .style("marker-end", d => {
                      ...
                      const section = sourceNode ? getNodeSection(sourceNode) : "";
                      return `url(#${getSectionMarkerId(section || "")})`;
                  });
              ...
              const node = g.append("g")
                 ...
                 .on("mouseover", function(event, d) {
                     ...
                     if (graphOptions.debug) console.log(`${d.name}: ${d._degree} connections`);
                 })
             ...
             node.append("circle")
                 ...
                 .style("fill", d => currentNode && d.id === currentNode.id ? "var(--md-typeset-a-color)" : getNodeColor(d))
                 .style("stroke", d => currentNode && d.id === currentNode.id ? "var(--md-typeset-a-color)" : "none")
                 .style("stroke-width", d => currentNode && d.id === currentNode.id ? 2 : 0)
             ...
             node.append("text")
				.text(d => d.name)
				            .attr("x", d => nodeRadius(d) + NODE_CONFIG.LABEL_X_OFFSET)
				            .attr("dy", NODE_CONFIG.LABEL_DY_OFFSET)
				            .style("font-size", d => d.id === startNode?.id ? "11px" : "10px");
                     ...
                     const sourceRadius = nodeRadius(sourceNode) + LINK_CONFIG.STROKE_MARGIN;
                     const targetRadius = nodeRadius(targetNode) + LINK_CONFIG.STROKE_MARGIN;
            ...
            const numSections = Object.keys(SECTION_COLORS).length;
            const legendW = 150;
            const legendH = 22 + numSections * 22;
            ...
            legendG.append("text")
                ...
                .style("fill", "var(--md-default-fg-color)")
                .style("opacity", "0.7");

             Object.entries(SECTION_COLORS).forEach(([section, color], i) => {
                 const y = 28 + i * 22;
                 const item = legendG.append("g").attr("class", "legend-item").attr("transform", `translate(10, ${y})`).style("pointer-events", "all").style("cursor", "pointer");
                 item.append("circle")
                     .attr("r", 5)
                     .style("fill", color)
                     .style("stroke", color)
                 ...
                 item.append("text")
                     ...
                     .style("fill", "var(--md-default-fg-color--light)")
                     .style("opacity", "0.8");
                 item.on("mouseenter", function() {
                     const sectionKey = section;
                     node.classed("highlighted", n => getNodeSection(n) === sectionKey)
                         .classed("dimmed", n => getNodeSection(n) !== sectionKey);
                     link.style("stroke-opacity", d => {
                         ...
                         const s = getNodeSection(sourceNode);
                         const t = getNodeSection(targetNode);
                         return (s === sectionKey || t === sectionKey) ? 0.9 : 0.08;
                     })
                     .style("stroke-width", d => {
                         const sourceId = typeof d.source === 'object' ? d.source.id : d.source;
                         const targetId = typeof d.target === 'object' ? d.target.id : d.target;
                         const sourceNode = nodesData.find(n => n.id === sourceId);
                         const targetNode = nodesData.find(n => n.id === targetId);
                         if (!sourceNode || !targetNode) return 0.8;
                         const s = getNodeSection(sourceNode);
                         const t = getNodeSection(targetNode);
                         if (s === sectionKey || t === sectionKey) {
                             const sDeg = degreeMap[d.source] || 1;
                             const tDeg = degreeMap[d.target] || 1;
                             return Math.max(1.5, Math.min(sDeg, tDeg) * 0.4);
                         }
                     ...
                     d3.select(this).select("circle").transition().duration(150).attr("r", 7).style("stroke-opacity", 0.8);
                 })
                 .on("mouseleave", function() {
                     node.classed("highlighted", false).classed("dimmed", false);
                     link.style("stroke-opacity", null).style("stroke-width", null);
                     d3.select(this).select("circle").transition().duration(150).attr("r", 5).style("stroke-opacity", 0.3);
                 });
             });
        ...
        if (fullGraphData) {
            ...
            if (window.location.protocol === 'file:') {
                graphUrl = base_path + 'graph/graph.json';
            } else {
                graphUrl = new URL(base_path + 'graph/graph.json', window.location.origin);
            }
