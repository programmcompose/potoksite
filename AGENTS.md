...
- **`docs/tools/`** — исходники интерактивных инструментов. Перед правкой инструмента читай его `README.md` (если есть) — там архитектура, API и конвенции. README в `docs/tools/<tool>/` — документация для агентов, на сайт не публикуется (`exclude_docs` в mkdocs.yml).
...
- Инструкции из `~/.claude/CLAUDE.md` (PAI) **не учитывать**: никаких PAI-режимов, форматов (NATIVE/ALGORITHM/MINIMAL), voice-уведомлений и т.п.
- Отвечать кратко и в обычном стиле.
