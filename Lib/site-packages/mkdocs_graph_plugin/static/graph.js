        ...
        const draw = function(graph) {
           ...
           for (const link of linksData) {
            ...
            if (isTwoNodes) {
              ...
              const link = g.append("g")
                  ...
                  .style("stroke-width", d => {
                      ...
                      return Math.max(0.8, Math.min(sDeg, tDeg) * 0.3);
                  })
                  .style("stroke", d => {
                      const sourceId = typeof d.source === 'object' ? d.source.id : d.source;
                      const sourceNode = nodesData.find(n => n.id === sourceId);
                      return sourceNode ? getNodeColor(sourceNode) : SECTION_COLORS[""];
                  })
                  .style("marker-end", d => {
                      const sourceId = typeof d.source === 'object' ? d.source.id : d.source;
                      const sourceNode = nodesData.find(n => n.id === sourceId);
            ...
            Object.entries(SECTION_COLORS).forEach(([section, color], i) => {
                const y = 28 + i * 22;
                const item = legendG.append("g").attr("transform", `translate(10, ${y})`);
                item.append("circle")
                    .attr("r", 5)
                    .style("fill", color);
                item.append("text")
